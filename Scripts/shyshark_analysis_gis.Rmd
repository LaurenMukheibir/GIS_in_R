...
I then read in the libraries I will need to start this project. Along the way I will add more.
```{r}
library(tidyverse)
library(rinat)
library(sf)
...
## Creating Plots 
Finding out where the data lies on a plot.
```{r}
...
Now plotting them with a base layer.
```{r}
library(rosm)
library(ggspatial)
ggplot() +
...
Using leaflet and htmltools, we will now create an interactive map for all three species.
```{r}
library(leaflet)
library(htmltools)
leaflet() %>%
